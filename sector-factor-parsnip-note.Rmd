...
Global_5_Factors <- 
 read_csv(
    file = unz(temp, "Global_5_Factors.csv"), 
    skip = 6, 
    # how to force column types
    col_types = cols(
      `Mkt-RF` = col_number(),
      SMB = col_number(),
      HML = col_number(),
...
print(tail(Global_5_Factors, n = 50), n = 50)

# Need to clean up the data for some reason there's annual factors in here too
```
...
Now let's move beyond the world of linear regression and examine how we could run our same monthly returns and factor data through a resampling and machine learning workflow. To do so, we will use three packages -  `parsnip`, `rsample` and `yardstick` - which are part of a new collection of packages called `tidymodels`. There is a lot to explore in `tidymodels` and we will just scratch the surface today.

Let's get started by reducing the data down to only the financials sector.
...
That `6` is telling us there are 6 months in the `analysis` set; that `1` is telling us that there is 1 month in our `assessment` data. The `216` is telling us there were 216 months in our original data set. Since we are using 6 months of data as our training data, we now have 210 splits.

Here is the `analysis` subset of that first split - it is 6 observations of monthly returns.
...
And here is the `assessment` subset - this is 1 month (specifically, it's the 7th month of our data since this is the first split).
...
Now we can start using that collection of data splits to fit and then assess our model.  That means it's time to introduce a relatively new addition to the R toolchain, the `parsnip` package.

`parsnip` is a unified model interface that allows us to create a model specification, set an analytical engine and then fit a model. It's a 'unified' interface in the sense that we can use the same scaffolding but insert different models, or different engines, or different modes.   

We will select the `ranger` package as the engine for our random forest model though it would be simple to change over to a different engine, such as the `randomForest` package.
...
Now, let's apply that random forest regression to all 210 of our splits (recall that each split consists of 6 months of training data and 1 month of test data), so we can get an average RMSE.
...
rf_regress <- function(mtry = 3, trees = 5, split){
    ...
    assessment_set_rf %>%
      ...
      select(date, monthly_return, .pred)
...
```{r}
results_rf <- 
  map_df(.x = financials_splits_2000_2017$splits,
...
Notice how the date of each prediction is included.
...
The power of `parsnip` is that we can quickly toggle to other model interfaces, such as `boost_tree()`, `decision_tree()`, or `mars()` and keep much the same workflow.  That's all for today and thanks for reading this note on Reproducible Finance.